...
public final class TabEffect {
    public static final int SLIDE=1,HAMMER=2,PULL=3,BEND=4,BEND_RELEASE=5,DEAD=6,HARMONIC=7,TAP=8;
    ...
    public static int encode(int kind,int semitones) {
        if(kind<1||kind>8||Math.abs(semitones)>24)throw new IllegalArgumentException("Unsupported guitar effect");
    ...
    public static String name(int marks){return switch(kind(marks)) {
        ...
        case BEND_RELEASE->"bend_release";case DEAD->"dead";case HARMONIC->"harmonic";case TAP->"tap";default->"none";};}
    ...
    public static double gain(int marks){return switch(kind(marks)){case DEAD->.25;case HAMMER,PULL,TAP->.78;default->1;};}
