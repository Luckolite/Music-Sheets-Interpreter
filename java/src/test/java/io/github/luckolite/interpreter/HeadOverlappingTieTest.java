...
/** Original synthetic ties whose shoulders begin underneath wide noteheads. */
public class HeadOverlappingTieTest {
    ...
    private Object note(int x, int step, float position, float scale) throws Exception {
        ...
        var head = headConstructor.newInstance(400, Math.round((x-12)*scale), Math.round((x+12)*scale),
    ...
    private boolean detect(int mode, int side, int step, float scale) throws Exception {
        ...
        for(int x=Math.round(103*scale);x<=Math.round(147*scale);x++) {
            float t=(x/scale-103)/44f;
        ...
        var result=(List<?>)mark.invoke(null,labels,gray,w,h,List.of(note(100,0,.2f,scale),note(150,step,.3f,scale)));
